...
bot=TelegramBot("Hier den API-Key von Botfather einfügen")
bot.gehe_online()
...
while (1):
    ...
    for nachrichtraw in nachrichten:
        ...
        if (nachrichtraw.inhalt != None and str(userid) not in blacklist):
            ...
            if (nachricht[0] == "/id"):
                send=("Chat-ID= "+ str(chatid, nachrichtid) + "\n Deine ID: " + str(userid))
                bot.sende_nachricht(send, chatid, nachrichtid)
            if (nachricht[0] == "/null"):
                bot.sende_nachricht("​", chatid, nachrichtid)
...
# Alles was mit Usergruppen zu tun hat
            if (nachricht[0] == "/stop"):
                if (str(userid) in admins):
                    ...
                    bot.sende_nachricht("Du bist kein Admin!", chatid, nachrichtid)
